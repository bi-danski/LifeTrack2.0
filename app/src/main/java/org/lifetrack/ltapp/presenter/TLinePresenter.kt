...
class TLinePresenter : ViewModel() {
    ...
    // Tree joints (e.g., "year_2024", "month_2024_10")
    ...
    fun toggleNode(nodeId: String) {
        _expandedNodes.update { if (it.contains(nodeId)) it - nodeId else it + nodeId }
    }

    fun updateNote(visitId: Int, note: String) {
        _patientNotes.update { it.toMutableMap().apply { this[visitId] = note.trim() } }
    }

    fun toggleSelection(visitId: Int) {
        _selectedVisitIds.update { if (it.contains(visitId)) it - visitId else it + visitId }
        _isSelectionMode.value = _selectedVisitIds.value.isNotEmpty()
    }

    fun toggleBookmark(visitId: Int) {
        _bookmarkedVisits.update { if (it.contains(visitId)) it - visitId else it + visitId }
    }
    ...
    fun openSearch() { _isSearchActive.value = true }
    fun onSearchQueryChange(query: String) { _searchQuery.value = query }
    fun clearSearch() { _searchQuery.value = ""; _isSearchActive.value = false }
    fun openFilterSheet() { _showFilterSheet.value = true }
    fun closeFilterSheet() { _showFilterSheet.value = false }
    fun setFilter(status: VisitStatus?) { _selectedFilter.value = status }
